...
export class PostsService {
  constructor(
    @Inject(POSTS_REPOSITORY) private readonly repository: PostsRepositoryPort,
  ...
  private async requireOwnedComment(userId: number, postId: number, commentId: number) {
    ...
    if (!isCommentAuthor(comment, userId)) throw new DomainException(AppErrorCode.FORBIDDEN);
    return comment;
  }
