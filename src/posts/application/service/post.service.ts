...
import { POST_REPOSITORY, type PostRepositoryPort } from '../port/post.repository.port';
...
export class PostService {
  constructor(
    @Inject(POST_REPOSITORY) private readonly repository: PostRepositoryPort,
  ...
  private async requireOwnedComment(userId: number, postId: number, commentId: number) {
    ...
    if (!isCommentAuthor(comment.authorId, userId)) {
      throw new DomainException(AppErrorCode.FORBIDDEN);
    }
    return comment;
  }
