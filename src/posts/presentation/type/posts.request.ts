...
export const createPostTransportSchema = z
  .object({ targetDate: z.unknown().optional(), contents: z.unknown().optional(), request: z.string().optional() })
  .strict();
...
export const postPageQuerySchema = z.object({ page: z.coerce.number().int().min(0).default(0), size: positiveIdSchema }).strict();
...
export const postCommentParamsSchema = z.object({ postId: positiveIdSchema, commentId: positiveIdSchema }).strict();
