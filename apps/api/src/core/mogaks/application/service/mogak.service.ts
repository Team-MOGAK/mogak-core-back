...
export class MogakService implements OwnedMogakPort {
  ...
  async getModarat(userId: number, modaratId: number) {
    ...
    if (modarat === null) throw new CoreError('MODARAT_NOT_FOUND');
    return {
      ...modarat,
  ...
  async updateModarat(
    ...
    if (updated === null) throw new CoreError('MODARAT_NOT_FOUND');
    return updated;
  }
  ...
  async deleteModarat(userId: number, modaratId: number): Promise<void> {
    if (!(await this.repository.deleteOwnedModarat(userId, modaratId))) {
      throw new CoreError('MODARAT_NOT_FOUND');
    }
  }
  ...
  async updateMogak(userId: number, mogakId: number, input: UpdateMogakCommand) {
    ...
    if (updated === null) throw new CoreError('MOGAK_NOT_FOUND');
  ...
  private async resolveCategory(
    ...
    if (selection.type === 'OFFICIAL') {
      ...
      if (category === null) throw new CoreError('MOGAK_CATEGORY_NOT_FOUND');
