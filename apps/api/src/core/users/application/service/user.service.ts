...
import type { SessionTokenIssuerPort } from '../../../auth/application/port/sessionTokenIssuer.port';
import { requiredTrimmed } from '../../../common/validation/requiredText';
import type { BinaryUpload, StoragePort } from '../../../storage/application/storage.port';
import {
...
export class UserService {
  ...
  async join(current: AuthenticatedPrincipal, command: JoinUserCommand): Promise<JoinUserResult> {
    ...
    if (job === null) throw new CoreError('JOB_NOT_FOUND');
    if (address === null) throw new CoreError('ADDRESS_NOT_FOUND');
  ...
  async profile(userId: number): Promise<UserProfileResult> {
    ...
    if (profile === null) throw new CoreError('USER_NOT_FOUND');
    return {
...
function requiredNickname(value: string): string {
  ...
  if (normalized === null) throw new CoreError('INVALID_PARAMETER');
  return normalized;
}
