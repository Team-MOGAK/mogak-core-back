...
`@nestjs/throttler`를 사용한다. `ThrottlerModule`은 기본 정책으로 IP별 300회/60초를 등록하고, `APP_GUARD`의 `ThrottlerGuard`를 전역 가드로 등록한다. 저장소만 `BoundedThrottlerStorage`로 교체해 키마다 고유한 만료 타이머를 두고, 만료 키를 삭제하며 최대 10,000개 키로 메모리를 제한한다. 패키지의 가드·데코레이터·429 응답은 그대로 사용한다. 인스턴스 세 대가 실행되면 요청은 라우팅에 따라 최대 약 세 배까지 통과하고 인스턴스 재시작 시 카운터가 초기화된다.
...
경로별 정책은 패키지 표준 `@Throttle({ default: { limit, ttl } })`로 전역 기본값을 덮어쓴다. 헬스 체크는 표준 `@SkipThrottle()`로 제외한다. 자체 가드·데코레이터는 두지 않는다. 저장소 어댑터는 키별 만료를 독립적으로 처리하므로 한 IP의 차단 해제가 다른 IP의 카운터나 만료 타이머를 바꾸지 않는다. 상한에 도달하면 만료 키를 먼저 제거하고, 모두 활성 상태면 가장 오래 사용하지 않은 키 하나를 제거한다.
